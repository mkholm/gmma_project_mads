...
# print("Initialize all ddG's to zero")
# subst$init_ddG = 0.0

settings$init_mean = 0.0
settings$init_sd = 0.5
print(sprintf("Initialize %d ddG's with normal distribution mean %.2f and sd %.2f", length(subst$init_m), settings$init_mean, settings$init_sd))
subst$init_ddG = rnorm(nrow(subst), settings$init_mean, settings$init_sd)

print(summary(subst$init_ddG))
...
# Guessing helps!
cp$B_D   = median(mutant[which(mutant$signal < cp$B_mid),"signal"])
cp$B_max = median(mutant[which(mutant$signal > cp$B_mid),"signal"])
print(sprintf("Initialize B_D %.2f and B_max %.2f from median of inactive and active", cp$B_D, cp$B_max))

cp$dG_wt = 0.0
print(sprintf("Initialize dG_wt %.2f",cp$dG_wt))

# signal global fit to optimize dG_wt by pretending it was optimized here
fit_wt = list(); fit_wt$par = list()
fit_wt$par$dG_wt = cp$dG_wt
...
# # # Multinomial density at average x
# # mn_maxdens = rep(NA,max(subst$obs))
# # for (n in seq(max(subst$obs))) {
# #     x_avg = n*p_inact
# #     while (sum(floor(x_avg)) < n) x_avg=x_avg+0.01   # Hack! may overshoot
# #     while (sum(floor(x_avg)) > n) x_avg=x_avg-0.0001 # Hack-hack! may undershoot
# #     x_avg = floor(x_avg)
# #     # If yet an undershoot (identical numbers in p_inact_mn), add a random sample
# #     if (sum(x_avg) < n) x_avg = x_avg + as.vector(rmultinom(1, n-sum(x_avg), p_inact))
# #     mn_maxdens[n] = dmultinom(x_avg,n,p_inact)    # for obs 1-3 this may be greater than 1
# # }
...
# # # Multinomial re-normalization to compare densities for different n
# # plot(log(subst$obs), log(subst$p_mn))
# # k = seq(max(subst$obs))
# # lines(log(k), -log(choose(30+k-1,k)), col=2)            # use this!
# # lines(log(k), -log(choose(30+k-1,k)*1e3), lty=2, col=2)
# # lines(log(k), log(mn_maxdens), col=3)
# # # None of them can keep up after log(20)~3 
...
# # NOTE that low likelihood may also be because _more_ are active than expected!
...
# # Obviously, the average of a bimodal distribution seem odd. The high degree of bimodality indicates that the transtion should be sharper. I.e. if variants are distributed
# # uniformly on the dG axis, the medium brightness region would be more populated.
# # Median, why? Colse to the average if variants are uniformly distributed on the brightness axis and very different with full bimodality e.g. binary readout. When I continue
# #   to the global analysis, the individual points are considered and not the average. There are very few points between the two modes where most of the average values lies.
# #   The median does fix this because it selects the value of the middle variant.
# # More generally I should formulate a error model (or prior), e.g. the beta function, but I'm still not sure which trait (parameter, momentum, etc.) from this distribution
# #   I should use to represent the function/fitness of a bimodal functioning collection of variants...
...
# # # Plot cleaning parameters 
# # quartz(height=5, width=9)
# # si_good = which(subst$init_m %in% c("1_curve", "2_curve_ddG1", "3_curve_ddG2"))
# # si_bad = which(subst$init_m %in% c("4_all_flat", "5_most_flat", "6_avg_individal"))
# # si_dis = which(subst$init_m %in% init_m_excluded_levels)
# # plot(subst$p_renom, subst$active, log="x", pch='.', xlim=c(1e-150,1e10), ylim=c(0,25),
# #      xlab="Likelihood of active/inactive based on N-mut frequencies", ylab="Active mutants (logF > 2.5)", main="Substitution clean parameters")
# # abline(h=c(settings$init_min_active,settings$ifs_min_active)-.5, v=settings$ifs_min_prob, col="lightgray")
# # points(subst[si_dis,'p_renom'], subst[si_dis,'active'], pch=1, col=4)
# # points(subst[si_good,'p_renom'], subst[si_good,'active'], pch=1, col=3)
# # points(subst[si_bad,'p_renom'], subst[si_bad,'active'], pch=1, col=2)
# # #points(subst[si_gmma,'p_renom'], subst[si_gmma,'active'], pch=4, col=1, cex=.5)
# # legend("topleft",c("Poor method","Good method","excluded"), col=c(2,3,4), pch=c(1,1,1), bg="white")
# # quartz.save("clean.png", type="png")

# # # Plot
# # quartz(height=5, width=9)
# # par(ask=T)
# # B_pred = function(dG) { e=exp((dG)/settings$RT); (cp$B_max+cp$B_D*e)/(1+e)}
# # x = seq(-10,10,.1)
# # mi_use = which(mutant$init == "use")
# # for (si in c(1544,1650,669,1506,sample(which(! is.na(subst$init_ddG))))) {
# #     mi = intersect(subst_mut_indices[[si]], mi_use)
# #     d = data.frame(mi=mi, B=mutant[mi,'signal'],
# #                    ddG = sapply(mut_subst_indices[mi], function(l) {sum(subst[l,'init_ddG'], na.rm=F)}), 
# #                    reparam = sapply(mut_subst_indices[mi], function(l) {length(unique(subst[l,'init_reparam']))-1}),
# #                    row.names=rownames(mutant)[mi])
# #     dG = d$ddG+cp$dG_wt # mutatnt$dG_init[mi]
# #     s = sprintf("%d: ddG_%s = %.2f kcal/mol. Obs=%d (of %d), m=%s",si,rownames(subst)[si],subst[si,'init_ddG'],length(mi),subst[si,'obs'],subst[si,'init_m'])
# #     plot(dG, d$B, xlim=c(min(dG,-3),max(dG,3)), ylim=c(min(d$B,cp$B_D),max(d$B,cp$B_max)), main=s)
# #     lines(x, B_pred(x))
# # }
# # # dG_wt gives the scal of the fit and thus resembles the m-value. Numeric larger dG_wt results in numeric larger ddG values
# # # More negative dG_wt in general results in steeper curve
