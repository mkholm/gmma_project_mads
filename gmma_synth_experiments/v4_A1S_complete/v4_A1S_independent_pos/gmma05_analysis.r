...
load("make_synthetic.rda") # load scan var 
if(!(dir.exists("global_fit"))){ 
  dir.create("global_fit")
}
png(sprintf("global_fit/global_fit_%f.png", extra$scan_var), width=6, height=4, units = "in", res = 200)


# png("global_fit.png", width=6, height=4, units = "in", res = 200)
x=seq(-20,20,.1)
...
plot(mutant$dG_glob, jitter(mutant$signal, factor = 0.5), pch=".", main=sprintf("Global fit, dG_wt %.2f",cp$dG_wt), xlab="Mutant stability [kcal/mol]", ylab="Brightness")
...
png("err_obs.png",height=6, width=6, units = "in", res = 200 )
...
png("ddG_hist.png", height=5, width=6, units = "in", res = 200)
# quartz(height=5, width=6)
hist(subst$ddG_glob, breaks=seq(settings$glob_ddG_min,settings$glob_ddG_max,.2), xlim=c(settings$glob_ddG_min,settings$glob_ddG_max), col="gray90", main="Global ddG fit")
